...
function typeDance(counter, speed, amplitude, hero_array) {
  ...
  // prettier-ignore
  let F_WDTH_RATE = Math.cos(counter * speed) ** steepness * -amplitude
  let R_WDTH_RATE = Math.sin(counter * speed) ** (steepness / 2) * -amplitude
  let J_WDTH_RATE = Math.sin(counter * speed) ** steepness * -amplitude
  let Z_WDTH_RATE = Math.cos(counter * speed) ** (steepness / 2) * -amplitude

  // prettier-ignore
  F_WDTH_RATE = gsap.utils.mapRange(-amplitude, amplitude, 63.6, 137.3, F_WDTH_RATE)
  // prettier-ignore
  R_WDTH_RATE = gsap.utils.mapRange(-amplitude, amplitude, 63.6, 137.3, R_WDTH_RATE)
  // prettier-ignore
  J_WDTH_RATE = gsap.utils.mapRange(-amplitude, amplitude, 63.6, 137.3, J_WDTH_RATE)
  // prettier-ignore
  Z_WDTH_RATE = gsap.utils.mapRange(-amplitude, amplitude, 63.6, 137.3, Z_WDTH_RATE)

  let remWidth_1 = totalWidth - (F_WDTH_RATE + R_WDTH_RATE)
  let remWidth_2 = totalWidth - (J_WDTH_RATE + Z_WDTH_RATE)

  let A_WDTH_RATE = remWidth_1 * 0.4
  // prettier-ignore
  A_WDTH_RATE = gsap.utils.mapRange(63.6, totalWidth - (3 * 63.6), 63.6, 177.3, A_WDTH_RATE)

  let O_WDTH_RATE = remWidth_1 * 0.6
  // prettier-ignore
